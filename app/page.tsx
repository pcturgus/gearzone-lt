...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchProducts(attempt = 1) {
      ...
      if (error) {
        console.error("fetchProducts klaida:", error.message, error);
        if (attempt < 3) {
          setTimeout(() => fetchProducts(attempt + 1), 800 * attempt);
          return;
        }
      }
    ...
    <div className="overflow-x-hidden w-full pb-16 md:pb-0">
      ...
      <footer className="hidden md:flex items-center justify-between max-w-[1440px] mx-auto px-8 py-6 text-xs text-[#9CA3AF] border-t border-[#E4E7EE] mt-6">
        ...
        <div className="flex items-center gap-5">
          ...
          <a href="https://www.instagram.com/pcturgus.lt" target="_blank" rel="noopener noreferrer" className="hover:text-[#5B4FE5]">Instagram</a>
        </div>
      </footer>
